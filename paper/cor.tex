...
I show in described above activity recognition models  features used to build well working systems.\\
...
As more examples are examined and feed throught the ILP new features are needed.\\
query language --- can just examine datasets by hand by quering them with designed features -> help to discovere dataset structure -> test and evaluate features.
At later these features can be used as feature extractors to build enhanced datasets---more informative\\



  \section{Discontinuity problem\label{sec:predictionDiscontinuity}}
Noise and incompletenes are great difficulty hen building model. 
...
in general by improving true positives and deproving false negatives.
...
Can be extended with activations window

  \section{Model generality}
Rule models are general
one example was applying sequential model to multi-label casee.

Models can be also easly transgferred between different datasets generated on different testbed
...
generality leads to possibiliti to apply real to synthetic and vise versa what is used as part of evaluation.
