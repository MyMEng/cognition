...
Presented above work shows the approach when activity details are available. Such datasets are rare as they need human anotation and activity preparation. In so far learnt models ... depend on item sensors and prior beliefe with litle use of motion sensors and location information.\\
In this section I address more common and more difficult unscripted data.


Addressed the easy case of scripted data\\blah blah blah about item sensr not being popular.
This problem is hard to tackle as nothing but the main objective of the activit is known.\\ mostly motion sensors used\\
here featrue design are more guesswork and intuition.\\

sensor are mixed. the task to distingish which sensro hoes to which\\
decide on sensor activation to resident assignment

    \subsection{Multi-label model for multiple residents\label{ch:mlm}}
one resident performs one activity at a time.\\
Two predictions to make---4 possibilities. multiple errors to make

    \subsection{Bathroom-toilet problem}
most common, only motion sensors\\
