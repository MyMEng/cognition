...
\citeauthor{plotkin1972automatic} in early 1970s and \citeauthor{shapiro1983algorithmic} in early 1980s created foundation of \emph{Inductive Logic Programming}(ILP). Both authors introduced techniques and tools to learn \emph{first-order formulas}---parametrised rules that are the core of ILP---from a set of facts. Since then, ILP techniques found number of interdisciplinary applications with new tools being created now and then for rules learning purposes.
...
Aleph is chosen based on its availablility and easy to use on contemporary platforms.

  \section{\texttt{ALEPH}}
Aleph is ILP writen in Prolog. it works wiht both YAP and SWI-Prolog implementation

    \subsection{Input files}
to learn a model it needs 3 files
...
indicate at which time points in data the activity was happening

      \subsubsection{Negatives}
in case of activcyt learning its a complement of positives examples bounded form below by $0$ and above by number of sensor reading in the data set.

    \subsection{The basic algorithm}
(page 4):

      \subsubsection{Select example}
select not covered example form positives
