...
Ilp system can only handel data in form o flogical facts therefore it needs to be represented like this

facts expressed in \texttt{Prolog} language.
...
as above
examples for both single resident and multiple residents
...
For each script of activities 10 data recordings are gathered together with corresponding model learnt with ILP.

Rules correcponding to each sample are tested on nine remaining samples and compared with ground truth to produce accuracy report on each label separatly and all labels combined. The output of cross-validation framework is on of the diagnostics used to evaluate model performance.

 capabilities on given set of activities
