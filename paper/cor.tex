...
In noisy data case there exists discontinuity of \texttt{locations} prediction space, which needs to be make continuous, unfortunately due to multiple \texttt{Prolog} results it cannot be done in the same way as in single resident scenario bias the location prediction toward last known location. A more advanced technique is needed.\\

In case two locations are returned use them. If less than two are returned use last known two locations. 
Locations = [room\_1].
...
Due to noise it is also possible that even though a person left a room the sensors are still activated, hence mote locations than nubmer of residents is lerurned. The idea is to check for sensor changes in each room in sliding time window 40 seconds rule length on average on -- if signal still the resident is out---remove his room---the longest standing sensor.
