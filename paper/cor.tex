...
Therefore, this simple model should work with just these two features. The basic building block of both of them is a \texttt{Prolog} predicat which interacts with sensor readings. Its main task is to check whether at given time T (input variable) what sensors are on (input/output variable). To make the feature more universal i.e.\ allow also to condition on turned off sensor third (input/output) variable stating state of sensor is introduced. Finally as there exist multiple time representations () in background knowledge input variable stating time type is needed of the form:\\
\begin{minted}[bgcolor=Bkgd,frame=leftline]{Prolog}
sensor_state(SensorID, SensorState, TimeType, Time) :-
  ruleBody.
\end{minted}
For simplicity of use in this particular case the rule can be wraped to provide interface with variables that we need:\\
\begin{minted}[bgcolor=Bkgd,frame=leftline]{Prolog}
sensor_state(SensorID, Time) :-
  sensor_state(SensorID, true, sequence, Time).
\end{minted}
In prolog a rule variable can be both input and output. If we query a rule it an return answer, or we can test correctness of belief---then such variables become input and prolog evaluates such statement to true or false.\\
As prolog is a search space it can be achieved by stating logical conditions warranting it.\\
The basic condition is that before or at time T\\

So it is straight forward to create features predicting current location and device that is on.\\
Remember about continuity fix: when all the sensors go out assume last known position---bias\\
\begin{minted}[bgcolor=Bkgd,frame=leftline]{Prolog}
location(Time, Location) :-
  ruleBody.
device(Time, Device) :-
...
Finally, designed feature rules are appended to the background knowledge file to be used by \texttt{Aleph}.

      \subsubsection{Configure \texttt{Aleph}}
The last step before runing the algorihtm is to put aleph configuration in background knowledge file preamble.\\
not pure rules\\
allow the rules to cover just one example -- as later they are purified. see point below\\
allow use \emph{determinations} and these


      \subsubsection{Learn the model}
Run ILP on 3 created file. Write learn rules to file. Purify them by removing singularities.

      \subsubsection{Evaluate results}
Eyball the rules whether they make sense. and I identified the right features.\\
They seem to work so we can proceed to more dificult case.\\
Rule coverage?? in comparisonwith majority class.
...
Despite the initial assumption of model being \emph{sequential} it is feasible to generalise it to multi-label overlapping case. As \texttt{Prolog} natively supports multiple answers to a query it is possible to apply previously learnt model to non-sequential scenario.\\
ETC.
...
% blah blah blah about item sensr not being popular.
    ...
    \subsection{bahroom excursion}
Two section explain: Movement case and Still case
