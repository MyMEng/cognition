...
Constructed model can be then used to analyse output from smart sites, therefore, describe ongoing human activities. Then, the model is extended to facilitate monitoring of multiple persons simultaneously e.g.\ multiple occupiers of a flat.\\
...
\section{Objective}
The main aim of the project is to apply Inductive Logic Programming to spatio-temporal data generated by smart installations like houses and cities to build \emph{Activity Recognition Model}. Such model describes each monitored activity with a set of \emph{first-order logical rules} built with events, where events are based on sensor readings. Example rules learn with \texttt{Prolog} are shown in \emph{Listing~\ref{lst:eg}}.
...
Once learnt, such model is useful in number of ways when making sense of data used to learn it.

  \subsection{Activity recognition}
The main purpose of building Activity Recognition Model is to build a tool capable of identifying actions taken by participant in time-window of interest. Rule based model allow to describe activities with different level of depth. The tool can also exploit the hierarchical structure of data: it can name the action, give building blocks of the action i.e.\ chain of sub-activities contributing to the main action, or state which sensor or sequence of sensors build such actions.
    
  \subsection{Distinguish multiple residents}
Most often monitored places are inhabited or being used by multiple persons. Another important task that can be performed with proposed technique is identifying actions and agents performing them when multiple users simultaneously share observed space.
...
All aforementioned applications of ILP are of great importance in healthcare. Being able to monitor patients who live at home and effectively describe their activities with ILP give better patient care possibilities. Moreover, when used in hospitals or daily care centres recognising multiple patients is desirable.\\
...
The concept of Inductive Logic Programming is based on fusion of two well known techniques: \emph{inductive learning} and \emph{logic programming}. The first component facilitates build model from available observation and generate new knowledge form experience. The latter introduces a powerful representation of knowledge as \emph{first-order logical rules}~\citep{muggleton1994inductive,muggleton1995inverse}.\\

First-order logic is more powerful than \emph{propositional logic} which builds sentences (i.e.\ rules) with facts only by using logical connectives. It extends propositional logic by using quantified variables (i.e.\ parameters) that belong to some domain of interest. The major limitation of first-order logic is its restriction to finite variable domains---it cannot describe rules build on infinite sets like real or natural numbers.\\ %  as opposed to

The power of ILP is inherited from first-order logical rules that it is built upon. First of all, it overcomes the use of ``limited representation formalism''---expressing data in form of a propositional logic. As many problems cannot be expressed propositionally, but it is possible in some form of first-order logic previously difficult to solve problems can be tackled.\\
...
The rules produced with ILP are human readable unlike models produced by majority of machine learning solutions what gives it great advantage over competition. Easy to understand rules mean that the logic models are arguably easy to manipulate. They can be changed by simply adding, deleting, and appending clauses or literals.\\
...
It is also popular in computer science: programming assistants tools, algorithmic debugging, program testing and verification, and reverse engineering~\citep{shapiro1983algorithmic,bergadno1993inductive,bratko1993inductive}.\\
...
Proposed here rules learning mechanism can be extended with ``grammar'' capable of generating \emph{Narrative Analysis} of spatio-temporal data: a \emph{succinct natural language description} of a selected time-space window of the data of interest. In healthcare and smart house scenarios, such techniques can be combined with activity recognition models to generate comprehensible descriptions of monitored activities with terminology and level of detail tuned to a particular recipient. In the context of the SPHERE project led by the University of Bristol these techniques would facilitate making sense from acquired data for healthcare applications in a more transparent way than is possible with black-box approaches.\\

\emph{Narrative Analytics} represents the data with succinct natural language description. Such form of data representation does not demand audience to have advanced analytic skills to extract information of interest. Therefore, proposed here tool can be of invaluable help while monitoring or assessing human activities in healthcare applications.\\[1em]

  \subsection{Introduction}
Most commonly used visualisation techniques nowadays are tables, graphs, and diagrams \citep{vis}. Unfortunately, such charts often need expertise in a field to be correctly interpreted. Moreover, aforementioned methods often trade-off clarity for completeness of shown data hence make it difficult for a broad audience to see the big picture.\\[1em]

The project aims at exploring possibility of describing data with natural language. This form of expressing information contained in the data has been under-appreciated and devoted little attention in recent years \citep{med,pat1,pat2}. Deep learning techniques have been applied to images to generate succinct scene description, nevertheless, it has not been used with raw data \citep{deep}.\\
An example of data narration can be found in modern smart-phone weather and calendar applications where instead of detailed weather forecast or hour-by-hour calendar events, one sentence description is presented to the user. Furthermore, in recent years \emph{narrative analytics} in combination with visualisation techniques have been proven as effective method of expressing corporate financial data \citep{quill}.\\[1em]

Successful adaptation of \emph{Narrative Analytics} to data provided by smart houses would greatly benefit healthcare applications. Being able to deliver activity description within given time window and with different level of detail would provide invaluable monitoring and diagnostic tool for daily care personnel and medical staff.


  \subsection{Challenges}
Creating a narrative of events is composed of two major components---``grammars'' needed to transform the data: \emph{data analysis} and \emph{narrative generation}.\\
The first one is deriving high-level knowledge from low-level data (sensor input). I aim to use \emph{Inductive Logic Programming} techniques to learn parametrised rules describing activity recognition model, hence, deduce performed activity \citep{muggleton1994inductive,muggleton1995inverse}. The latter step is ``translating'' extracted information into natural language description, which also can be done using similar approach \citep{narr}.\\[1em]

Very often the data collected from smart houses is corrupted or obscure due to sensor noise, leading to confusing gaps between ``observed'' actions. With help of inductive learning such implicit information---describing intermediate steps between actions---can be inferred to overcome rule creation difficulty and produce logically consistent scenario. This in turn can be translated into natural language using second ``grammar''.\\[1em]
...
Furthermore, proposed here descriptive mechanisms could be extended to become a query-enabled database. In such model a user could ``ask'' a question in natural language to get narrative answer instead of numerical vector.\\[1em]
...
The spatio-temporal class of data are of interest. Data-points in such sets are characterised by both location and time of the event. Such data are generated by all smart instillations like houses and cities. In this study smart house data are used but the generalisation of the solution is straight forward.\\ % specification
...
Participants performed five \emph{Activity of Daily Living}(ADL) activities:
\begin{description}
\item[Make a phone call] Move to the phone in the dining room; find a specific number in the phone book; dial the number; listens to the message; summarize cooking directions on a notepad provided over the phone.
\item[Wash hands] Move into the kitchen; wash hands in the sink with hand soap; dry hands with a paper towel.
\item[Cook] Cooks a pot of oatmeal according to the directions given in the phone message: measure water, pour the water into a pot; boil the water; add oats; put the oatmeal into a bowl; add raisins and brown sugar.
\item[Eat] Take cooked oatmeal; take medicine container; move to the dining room; eat the food.
\item[Clean] Take all of the dishes to the sink in kitchen; clean them with water and dish soap.
\end{description}
